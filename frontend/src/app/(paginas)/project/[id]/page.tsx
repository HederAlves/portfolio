...
export default async function PaginaProjeto(props: { params: Promise<{ id: string }> }) {
	...
	const readme = await obterReadme(projeto.repository)

	return (
		<div className="bg-black">
			<Cabecalho />
			<Container className="py-7 flex flex-col items-center gap-10">
				<h1 className="text-3xl font-bold self-start">{projeto.name}</h1>
				<CarrosselImagens imagens={projeto.images.slice(1)} />
				<Tecnologias lista={projeto.technologies} tamanhoMenor />
