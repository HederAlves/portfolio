...
export default function JanelaChat() {
	const { message, pensando, adicionarMensagem, limparMensagens } = useChat()
	...
	return (
		<div className="flex flex-col bg-zinc-300 rounded-2xl text-black overflow-hidden">
			...
			{message.length === 0 ? (
				...
				<div className="flex flex-col p-2 gap-2 min-h-[400px] sm:min-h-[300px] max-h-[300px] sm:max-h-[300px] overflow-y-scroll">
					{message.map((mensagem, i) => {
						const mesmoAutor = i > 0 && message[i - 1].autor === mensagem.autor
						return (
							<BalaoMensagem
								key={mensagem.id}
								message={mensagem}
